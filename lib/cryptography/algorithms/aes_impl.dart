...
class _AesCtr extends Cipher {
  ...
  List<int> decryptSync(
    ...
    if (keyStreamIndex != 0) {
      nonceBytes = Uint8List.fromList(Nonce(nonceBytes).increment(keyStreamIndex ~/ 16).bytes);
    }
    ...
    for (var i = 0; i < input.length; i += 16) {
      ...
      // Increment nonce
      nonceBytes = Uint8List.fromList(Nonce(nonceBytes.toList()).increment().bytes);
    }
    return output;
  }
  ...
  List<int> encryptSync(
    ...
    nonceBytes.setAll(0, nonce.bytes);
    if (keyStreamIndex != 0) {
      nonceBytes =Uint8List.fromList(Nonce(nonceBytes.toList()).increment(keyStreamIndex ~/ 16).bytes);
    }
